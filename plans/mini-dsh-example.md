...
│   └── driver.py                   # 组合根：create Session + ReactLoopAgent → provide session/agent
└── providers.py                    # ScriptedAdapter（4 场景工厂）+ OpenAICompatAdapter
```

代码规模目标：core ~1.1k + 外围（plugins/providers/cli）~0.7k ≈ **1.8k 行以内**。
...
| `session.py` + `inbox.py` | 保留事件日志/derive_messages/双队列；砍 on_append 宿主钩子 |
...
### 装配模型（cordis，收敛到 4 插件）

- 服务名沿用 dsh 词汇：`llm` / `tools` / `agentLoop` / `session` / `agent`。
...
- driver 插件 `inject=[llm, tools, agentLoop]`，构造 Session + ReactLoopAgent
  （`AgentOptions(provider=…, model=…)` 作初始路由 seed）；同时
  `provide("agentLoop")`：`AgentLoopConfig(max_parallel_tool_calls=2)`（tools 场景的
  并行池上限，core/tools 调度从 `ctx.get("agentLoop")` 读取）。
...
- `examples/plugin_harness/` 删除；新 `examples/mini_dsh/`。
