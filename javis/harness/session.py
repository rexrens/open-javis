...
class Session:
    ...
    The optional ``on_append(seq, type, data)`` callback fires synchronously
    after every successful append — the host engine uses it to wake its
    event bridge (the demo's copy does not have this hook).
    """

    def __init__(self, id: SessionId, cwd: str | None = None, on_append=None) -> None:
        self.id = id
    ...
    def append(self, type: str, data: dict[str, Any] | None = None, **options: Any) -> SessionEvent:
        ...
        if self.on_append is not None:
            self.on_append(event.seq, event.type, event.data)
        return event
