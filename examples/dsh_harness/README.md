...
> 与 [`examples/cordis_harness`](../cordis_harness/README.md) 的对照：
> 本示例的生产 core 是 `javis.harness`（生产核心装配）；
> cordis_harness 是**独立自包含的最小可用 harness**（外部只依赖 `javis.cordis`）——
...
## 与 cordis_harness 的对照（两种引擎姿势）

| 维度 | 本示例（dsh_harness） | examples/cordis_harness |
|---|---|---|
| 引擎 core | **javis.harness**（生产核心，完整契约面 + 宿主集成） | **独立自包含的最小 harness**（外部只依赖 `javis.cordis`） |
| core 代码 | 生产包本身（`javis/harness/`） | `examples/cordis_harness/harness/` |
| 插件角色 | 提供引擎的每个部件（llm/agentTools/systemPrompt/agentLoop 都是插件 provide） | 每个能力一行插件（llm/sessions/tools/agent/cli），组合文件即可替换 |
| 宿主 | 自持 cli.py（4 场景） | 自持 cli.py（交互 REPL + `--dump-config` + `--resume`） |
| 配置 | 组合文件 | 组合文件 + **分层 patch**（`cordis.patch.yml`，不改 base 加插件） |
| 定位 | 生产 core 装配（生产） | 最小可用 harness（示例/教学位） |
